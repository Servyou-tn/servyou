...
export type SearchOutcome = {
  ...
  perPage: number // page size actually applied (services override the shared default)
}
...
function applyServiceFilters<Q extends AnyQuery>(
  ...
  let out = applySearch(q, params.q)
  if (categoryIds) out = out.in('category_id', categoryIds) as Q
  // City filter — freelancer_profiles is an !inner embed, so filtering its city narrows the
  // parent services to those whose freelancer is in the selected city.
