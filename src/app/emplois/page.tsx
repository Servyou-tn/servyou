...
import { JobBoardFiltersBar, type CityOption, type JobFilterSelection } from '@/components/freelance/JobBoardFiltersBar'
...
// PUBLIC — no auth/role gate (browsing is open to everyone, per product.md). Layout dispatches by
// role so the FreelancerSidebar appears on EVERY freelancer page (the locked sidebar-persistence
// rule in the design constitution): a freelancer gets FreelancerLayout (its 6-item sidebar);
// everyone else gets MarcheLayout (the consumer sidebar). Filters render as a horizontal bar above
// the cards (Indeed/LinkedIn pattern) so neither shell needs a competing filter column. The two
// shells differ in PageHeader ownership — FreelancerLayout renders it from subtitle/emphasisWord;
// MarcheLayout does not, so the MarcheLayout branch mounts <PageHeader> itself. Detail route
// /emplois/[id] ships later (card links 404 until then).
export default async function EmploisPage({
  ...
  const subtitle = t('page_header.emplois.subtitle', lang)
  const emphasis = t('page_header.emplois.emphasis', lang)

  // The shared page body (filter bar + count/sort + results), rendered inside whichever shell.
  const content = (
    <>
      <JobBoardFiltersBar
      ...
      {!isEmpty && (
        <div className="mb-4 flex items-center justify-between gap-3">
          <p className="text-sm text-text-muted">{t('emplois.results_count', lang, { count: total })}</p>
      ...
      {isEmpty ? (
        <div className={`rounded-2xl bg-white p-12 text-center ${CARD_SHADOW}`}>
          <div className="mx-auto max-w-md">
            ...
            <p className="mt-4 text-base font-semibold text-text-primary">{t('emplois.empty.title', lang)}</p>
            <p className="mt-2 text-[13px] text-text-muted">{t('emplois.empty.description', lang)}</p>
            {hasFilters && (
              <Link
                href="/emplois"
  ...
  // Freelancer → FreelancerLayout (owns PageHeader). Everyone else → MarcheLayout (mount it here).
  if (isFreelancer) {
    return (
      <FreelancerLayout user={topBarUser} subtitle={subtitle} emphasisWord={emphasis}>
        {content}
      </FreelancerLayout>
    )
  }
  ...
  return (
    <MarcheLayout user={topBarUser}>
      <PageHeader subtitle={subtitle} emphasisWord={emphasis} />
      {content}
    </MarcheLayout>
  )
}
