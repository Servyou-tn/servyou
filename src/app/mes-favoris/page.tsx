  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-10">
      <div className="max-w-4xl mx-auto space-y-10">
        ...
        {/* Products */}
        <section>
          <h2 className="text-lg font-semibold text-gray-700 mb-3">Produits ({productFavs.length})</h2>
          {productFavs.length === 0 ? (
            <p className="text-gray-400 text-sm">Aucun produit en favoris.</p>
          ) : (
        ...
        {/* Services */}
        <section>
          <h2 className="text-lg font-semibold text-gray-700 mb-3">Services ({serviceFavs.length})</h2>
          {serviceFavs.length === 0 ? (
            <p className="text-gray-400 text-sm">Aucun service en favoris.</p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {serviceFavs.map(f => {
                const s = f.service!
