...
// Manrope, scoped via --font-display — the same display voice as the hero /
// categories / how-it-works so the signup funnel reads as one piece with the
// landing page. No 'use client': this page is pure presentation + routing
// (hover lift, focus ring and reduced-motion are all CSS, <Link> works
// server-side), so it ships zero JS.
...
const ROLE_CARDS = [
  // TODO: /inscription/consumer route is a placeholder — Step 2 form built next.
  { key: 'consumer', href: '/inscription/consumer', icon: '/brand/icons/icon-cart.png' },
  // TODO: /inscription/shop-owner route is a placeholder — Step 2 form built next.
  { key: 'shopOwner', href: '/inscription/shop-owner', icon: '/brand/icons/icon-shop.png' },
  // TODO: /inscription/freelancer route is a placeholder — Step 2 form built next.
...
export default async function InscriptionPage() {
  ...
  return (
    // Full-height white canvas. The Header is hidden on /inscription (chromeless,
    // see selectVariant) so this page owns the whole viewport and carries its own
    // minimal logo-only navbar to keep the focus on the 3 role cards.
    <div className={`${manrope.variable} relative flex min-h-screen flex-col bg-[var(--surface-base)]`}>
      {/* Subtle dot texture (3%) — the how-it-works pattern in brand-sky, barely
          there, for a touch of warmth over pure white. Mirrors automatically. */}
      <div
        aria-hidden="true"
      ...
      {/* Minimal navbar — logo only, no menu / no auth CTAs. Sits at the inline
          start (top-left LTR, top-right RTL via the page dir). */}
      ...
      >
        ...
        <div className="w-full max-w-[720px] rounded-3xl border border-[var(--border-subtle)] bg-white p-8 shadow-[0_24px_64px_rgba(15,23,42,0.08)] md:p-14">
          <h1
          ...
          {/* 3 role cards: stacked on mobile, a 3-up grid on md+. The grid's
              inline axis reverses automatically under <html dir="rtl">. */}
          ...
          >
            ...
            {/* TODO: /connexion route not built yet (current sign-in is /login). */}
            <Link
              href="/connexion"
