...
export default function ServicesPage() {
  ...
  async function handleDelete(id: string, title: string) {
    if (!window.confirm(t('freelance.delete_service_confirm', lang, { title }))) return
    ...
    <main className="min-h-screen bg-gray-50 px-4 py-12">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-800">{t('freelance.services_title', lang)}</h1>
          <Link
          ...
          >
            {t('freelance.add_service_btn', lang)}
          </Link>
        </div>

        {services.length === 0 ? (
          <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
            <p className="mb-4">{t('freelance.no_services_own', lang)}</p>
            <Link href="/mon-profil-freelance/services/nouveau" className="text-blue-600 hover:underline text-sm">
              {t('freelance.no_services_cta', lang)}
            </Link>
          </div>
        ) : (
