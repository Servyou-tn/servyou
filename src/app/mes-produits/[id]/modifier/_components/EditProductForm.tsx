...
export function EditProductForm({
  ...
  // status, so there is no second place that could disagree with it. The header title row below
  // reads this SAME state (not product.status) so it never disagrees with the footer's own link.
  ...
  return (
    <>
      {/* Header title row — Figma 535:32433 draws H1 + StatusPill + "Voir le produit" together,
          space-between (independently confirmed by the H7 sibling's identical header pattern).
          Lives here, not in page.tsx, because it has to track the SAME live `status` state as the
          footer's link — a server-rendered header can't react to the §5 toggle without a reload.
          Stacked below sm: H1 "Modifier le produit" + a pill is already long in FR, a second
          full-width link on the same row would overflow well before 375px. */}
      <div className="mb-8 max-w-[760px]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-2xl font-semibold text-text-primary">{t('product.edit_title', lang)}</h1>
            <StatusPill status={status === 'active' ? 'active' : 'masqué'}>
              {t(status === 'active' ? 'common.status_active' : 'common.status_hidden', lang)}
            </StatusPill>
          </div>
          {/* Same visibility rule as the footer's link: only when live status is 'active' (D1
              404s a Masqué product). */}
          {status === 'active' ? (
            <Link
              href={`/produits/${product.id}`}
              className={`inline-flex w-fit items-center gap-1.5 rounded text-sm font-semibold text-brand-blue-600 hover:underline ${FOCUS_RING}`}
            >
      ...
      >
        ...
        <section className={section}>
          ...
          <div className="space-y-5">
            ...
            {/* Full width, not a two-col grid — that shape was a leftover from cloning G6's
                Catégorie+Sous-catégorie layout. There is no subcategory field to sit beside it
                (categories is a flat, single-level table; see product-categories.ts), so a 2-col
                grid with one child just left dead space at sm+. Ruled, not a gap — do not add
                Sous-catégorie here. */}
            <div>
        ...
        <div className={`sticky bottom-0 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-5 ${CARD_SHADOW}`}>
          <p className="flex items-center gap-2 text-sm text-text-muted">
            {isDirty ? (
              <>
                <span aria-hidden="true" className="h-1.5 w-1.5 shrink-0 rounded-full bg-warning-500" />
                {t('product.edit.dirty', lang)}
              </>
            ) : null}
          </p>
