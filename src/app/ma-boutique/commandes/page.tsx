  ...
  return (
    <main className="min-h-screen bg-gray-50 px-4 py-12">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-800 mb-6">Commandes reçues</h1>
              ...
              return (
                <div key={o.id} className="bg-white rounded-lg shadow p-5 space-y-3">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {o.profiles?.full_name ?? 'Acheteur'} · Qté : {o.quantity} · {date}
                      </p>
                    </div>
                  ...
                  <div className="text-sm text-gray-600 space-y-0.5">
                    {o.delivery_name && <p><span className="font-medium">Livraison :</span> {o.delivery_name}</p>}
                    {o.delivery_address && <p><span className="font-medium">Adresse :</span> {o.delivery_address}</p>}
                    {o.delivery_phone && <p><span className="font-medium">Tél :</span> {o.delivery_phone}</p>}
                    {o.buyer_note && <p><span className="font-medium">Note :</span> {o.buyer_note}</p>}
                  </div>
        ...
        <div className="mt-6">
          <Link href="/ma-boutique" className="text-sm text-blue-600 hover:underline">← Retour à ma boutique</Link>
        </div>
      </div>
    </main>
  )
