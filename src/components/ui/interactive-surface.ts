...
// The resting surface: white card with a subtle border + shadow, dark text.
export const SURFACE_IDLE = 'bg-white border border-border-subtle shadow-sm text-text-primary'

// The hover lift (carries the transition so unhovered → hovered is smooth).
export const SURFACE_HOVER = 'transition-all duration-200 ease-out hover:bg-slate-50 hover:shadow-md'

// The selected treatment — a light brand-accent tint (Linear / Vercel / Stripe pattern), NOT a
// solid fill. A selected surface is stable: it keeps the tint, no hover swap.
export const SURFACE_ACTIVE = 'bg-brand-accent/10 border-brand-accent/40 text-brand-accent shadow-sm'
