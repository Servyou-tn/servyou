...
Several schema migrations are sequenced for pre-launch but not yet applied:

- **Shop owner configurable workspace fields.** Adding `shop_type`, `delivery_setup`, `working_hours`, `location_detail` columns to the shops table. Adding the `shop_payment_methods` and `shop_categories` child tables.
- **Freelancer configurable workspace fields.** Adding `working_hours`, `current_workplace`, `preferred_payment_method` columns to the freelancer_profiles table. Adding the `freelancer_tools`, `freelancer_education`, and `freelancer_certifications` child tables.
- **Buyer-cancellation-history view.** A read-only view aggregating cancellations per buyer for the post-launch buyer-rating system. The view is built at MVP; the rating system that consumes it is post-launch.

Each migration ships as a focused PR: discovery via `execute_sql`, draft SQL, founder authorization, `apply_migration`, verification, mirror to `/db/migrations/`, then the application-layer UI to expose the new fields. The UI changes are themselves focused PRs separate from the migration PR.
